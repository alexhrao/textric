...
export function isClientMessage(msg: unknown): msg is ClientMessage {
    if (typeof msg !== 'object' || msg === null || msg === undefined) {
        ...
        (typeof (<ClientMessage>msg).dst === 'string' ||isAddress((<ClientMessage>msg).dst)) &&
...
export function isReplyMessage(msg: unknown): msg is ReplyMessage {
    if (typeof msg !== 'object' || msg === null || msg === undefined) {
        ...
        'body' in msg &&
        'ref' in msg &&
        typeof (<ReplyMessage>msg).id === 'number' &&
        typeof (<ReplyMessage>msg).timeLocal === 'number' &&
        typeof (<ReplyMessage>msg).body === 'string' &&
        // typeof (<ReplyMessage>msg).ref === who knows...idk how to check arrays
        (<ReplyMessage>msg).type == MessageType.RPLY
    ) {
        return true;
    } else {
        return false;
...
export function isAuthMessage(msg: unknown): msg is AuthMessage {
    if (typeof msg !== 'object' || msg === null || msg === undefined) {
        ...
        'type' in msg &&
        (<AuthMessage>msg).type == MessageType.AACK
    ) {
        return true;
    } else {
        return false;
...
export function isAliveMessage(msg: unknown): msg is AliveMessage {
    if (typeof msg !== 'object' || msg === null || msg === undefined) {
        ...
        'type' in msg &&
        (<AliveMessage>msg).type == MessageType.ALIV
    ) {
        return true;
    } else {
        return false;
...
export function isDatabasePayload(payload: unknown): payload is DatabasePayload {
