...
export function isDEInit(init: unknown): init is DEInit {
    if (typeof init !== 'object' || init === null || init === undefined) {
        return false;
    } else if ('userID' in init && 'deviceID' in init && Object.keys(init).length === 2) {
        return true;
    } else {
        return false;
    }
...
export function isServerHash(hash: DeviceHashPayload|ServerHashPayload): hash is ServerHashPayload {
    if ('passHash' in hash) {
        return true;
    } else {
        return false;
